...
Erizo.ChromeRoapStack = function(spec) {
  ...
  L.Logger.debug("Created webkitRTCPeerConnnection with config \"" + JSON.stringify(
    that.pc_config) + "\".");
    ...
    if (that.actionNeeded) {
      if (that.state === 'new' || that.state === 'established') {
        ...
        that.peerConnection.createAnswer(function(sessionDescription) {
          ...
          if (!that.iceStarted) {
            var now = new Date();
            L.Logger.debug(now.getTime() +
              ': Starting ICE in responder');
        ...
        that.error('Dazed and confused in state ' + that.state +
          ', stopping here');
      }
