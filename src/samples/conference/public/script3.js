var runRestSample = function () {
  ...
  var portal = '';
  var participant_id;
  var query_period = 1000, query_interval;
  var published = {};
  var subscribed = {};
  var mixed_stream;

  function send (method, url, body, on_ok, on_error) {
    var req = new XMLHttpRequest({rejectUnauthorized: false});
  ...
  function join (token, on_ok, on_error) {
    ...
    var body = {token: token, queryInterval: query_period};
  ...
  function publish (clientId, type, options, on_ok, on_error) {
    ...
    var body = {type: type, options: options};
  ...
  var onQueryResult = function (result) {
    ...
    for (var stream_id in result.published) {
      result.published[stream_id].map(function(signaling) {handlePubSignaling(stream_id, signaling);});
    }

    for (var subscription_id in result.subscribed) {
      result.subscribed[subscription_id].map(function(signaling) {handleSubSignaling(subscription_id, signaling);});
    }

    result.notifications.map(function(evt) {handleNotification(evt);});
  };
  ...
  var handleSubSignaling = function (subscriptionId, signaling) {
    switch (signaling.type) {
      ...
      case 'candidate':
      default:
        L.Logger.debug('subscription:', subscriptionId, 'signaling:', signaling);
    }
  };
  ...
  function trySubscribeStream (stream) {
    if (stream instanceof Woogeen.RemoteMixedStream) {
      ...
      if (subscribeMix === 'true') {
        ...
        subscribe(participant_id, 'webrtc', {audio: {from: stream.id}, video: {from: stream.id, resolution: resolution}}, function (sub) {
          L.Logger.info('subscribed:', stream.id, 'and subscription id is:', sub.id);
      ...
      if (subscribeMix !== 'true' || stream.isScreen()) {
        L.Logger.info('subscribing:', stream.id());
        subscribe(participant_id, 'webrtc', {audio: {from: stream.id}, video: {from: stream.id}}, function (sub) {
          L.Logger.info('subscribed:', stream.id, 'and subscription id is:', sub.id);
  ...
  window.onload = function () {
    ...
    if (isHttps) {
      ...
      if (shareButton) {
        ...
        shareButton.onclick = (function () {
          publish(participant_id, 'webrtc', {audio: false, video: {device: 'screen', resolution: myResolution}}, function (st) {
    ...
    externalOutputButton.onclick = (function () {
      ...
      return function () {
        ...
        if (!isOutputing) {
          subscribe(participant_id, 'url', {url: url.value, audio: {from: mixed_stream}, video: {from: mixed_stream}}, function (sub) {
    ...
    createToken(myRoom, 'user', 'presenter', function (response) {
      ...
      join(token, function (resp) {
        if (typeof mediaUrl === 'string' && mediaUrl !== '') {
          publish(participant_id, 'url', {url: mediaUrl, transport: 'udp', bufferSize: 2048}, function (st) {
          ...
          if(isPublish !== 'false') {
            Woogeen.LocalStream.create({
              ...
              publish(participant_id, 'webrtc', {audio: true, video: {device: 'camera', resolution: myResolution}}, function (st) {
